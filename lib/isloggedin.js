...
const passportStrategy = function() {
  ...
  return new BasicStrategy(
	  function(userid, password, done) {
	  	...
	  	// invalid user
	  	if (typeof process.env.SOS_LOCKDOWN_USERNAME !== 'string' || typeof process.env.SOS_LOCKDOWN_PASSWORD !== 'string') {
	  		return done(null, false)
	  	}

	  	// supplied username/password don't match
	  	// invalid user
	    if (userid != process.env.SOS_LOCKDOWN_USERNAME || password != process.env.SOS_LOCKDOWN_PASSWORD) { 
	    	return done(null, false);
	    }
